...
def xavier_weight_init():
  ...
  def _xavier_initializer(shape, **kwargs):
    ...
    out = tf.random_uniform(shape, -init_range, init_range)
...
def test_initialization():
  ...
  assert xavier_mat.get_shape() == shape
  session = tf.Session()
  print 'Expected Xavier initializer: ', np.sqrt(6.0/sum(shape))
  print 'Actual Xavier initializer: ', np.max(session.run(xavier_mat))
  #raise NotImplementedError
  ### END YOUR CODE  
