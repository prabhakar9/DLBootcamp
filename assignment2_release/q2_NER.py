...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder = tf.placeholder(tf.int32, (None, self.config.window_size), 'input')
    self.labels_placeholder = tf.placeholder(tf.float32, (None, self.config.label_size), 'labels')
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer") as layer_scope:
        input_dims = self.config.window_size*self.config.embed_size
