...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    conv_out, conv_cache = conv_relu_forward(X, self.params['W1'], self.params['b1'], conv_param)
    pool_out, pool_cache = max_pool_forward_fast(conv_out, pool_param)
    ar_out, ar_cache = affine_relu_forward(pool_out, self.params['W2'], self.params['b2'])
    scores, a_cache = affine_forward(ar_out, self.params['W3'], self.params['b3'])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx2_out = max_pool_backward_fast(dx2, pool_cache)
    dx1, grads['W1'], grads['b1'] = conv_relu_backward(dx2_out, conv_cache)
